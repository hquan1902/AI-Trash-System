...
def main():
    ...
    for source_idx, (ds, split, json_name, name_map) in enumerate(train_sources):
        ...
        print(
            f"  - {ds}/{split}: images={len(remapped['images'])}, "
            f"anns={len(remapped['annotations'])}, dropped={remapped['dropped']}, "
            f"cls={dict(remapped['class_counts'])}"
        )
    ...
    print(
        f"  - train saved: {train_json} | images={len(out_train['images'])}, "
        f"anns={len(out_train['annotations'])}, cls={_count_by_class(out_train)}"
    )
    print(
        f"  - val saved  : {val_json} | images={len(out_val['images'])}, "
        f"anns={len(out_val['annotations'])}, cls={_count_by_class(out_val)}"
    )
    ...
    print(
        f"  - test saved : {test_json} | images={len(out_test['images'])}, "
        f"anns={len(out_test['annotations'])}, cls={_count_by_class(out_test)}"
    )
